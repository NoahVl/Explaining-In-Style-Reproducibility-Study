...
class FFHQ(data.Dataset):
    def __init__(self, root, label="gender", pre_transform=None):
        """
        ...
        # Store image paths
        self.images = [os.path.join(self.root, "data", file)
                       for file in os.listdir(os.path.join(self.root, "data")) if file.endswith('.jpg')]
