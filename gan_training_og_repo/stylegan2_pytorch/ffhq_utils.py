...
from ffhq_data_loader import FFHQ
...
def get_train_valid_test_resized_dataset(data_dir, label, valid_ratio=0.15, test_ratio=0.15, resize_size=32):
    dataset = FFHQ(data_dir, label)
    train_length, valid_length, test_length = int(len(dataset) * (1 - valid_ratio - test_ratio)), \
                                              int(len(dataset) * valid_ratio), int(len(dataset) * test_ratio)
    # Make sure that the lengths sum to the total length of the dataset
    ...
    second_dataset = FFHQ(data_dir, label, pre_transform=transforms.Compose([transforms.Resize(resize_size)]))
    _, _, test_dataset_resized = torch.utils.data.random_split(second_dataset,
                                                               [train_length, valid_length, test_length],
                                                               generator=torch.Generator().manual_seed(42)
                                                               )

    return train_dataset, val_dataset, test_dataset, test_dataset_resized
