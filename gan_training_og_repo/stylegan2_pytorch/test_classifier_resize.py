...
def load_model(model, checkpoint_name):
    ...
    model.load_state_dict(torch.load(os.path.join("saved_classifiers", checkpoint_name), map_location=device))
    return model
...
def test_model(model, batch_size, device, seed, test_dataset, resized_dataset=None):
    """
    ...
    with torch.no_grad():
        if resized_dataset:
            test_loader_resized = DataLoader(resized_dataset, batch_size=batch_size,
                                     generator=torch.Generator().manual_seed(42), pin_memory=True,
                                     num_workers=6)
            test_resized_accuracy = evaluate_model(model, test_loader_resized, device)
            test_results['accuracy_resized'] = test_resized_accuracy.item()
            print(f"Resized test accuracy: {test_resized_accuracy.item():.4f}")

            # Plot some of resized_dataset in matplotlib
            import matplotlib.pyplot as plt
            plt.imshow(resized_dataset[0][0].permute(1, 2, 0))
            plt.show()
...
def main(args: argparse.Namespace):
    ...
    if args.dataset == "FFHQ-Aging":
        train_dataset, valid_dataset, test_dataset, test_resized = ffhq_utils.get_train_valid_test_resized_dataset(
            "", "gender", resize_size=32)
    else:
    ...
    if not os.path.exists(os.path.join("saved_classifiers", args.checkpoint_name)):
    ...
    # Return the results
    test_results = test_model(model, args.batch_size, device, args.seed, test_dataset, resized_dataset=test_resized)
